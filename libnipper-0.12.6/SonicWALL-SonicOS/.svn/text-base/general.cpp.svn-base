...
// Initialisation...
SonicOSGeneral::SonicOSGeneral()
{
}


int SonicOSGeneral::processDeviceConfig(Device *device, ConfigLine *command, char *line, int lineSize)
{
	// Hostname
	if (strcmp(command->part(0), "firewallName") == 0)
	{
		...
		hostname.assign(strstr(line, command->part(1)));
	}
	...
	{
		if (device->config->reportFormat == Config::Debug)
			printf("%sSerial Number Line:%s %s\n", device->config->COL_GREEN, device->config->COL_RESET, line);
		serialNumber.assign(command->part(1));
	}
...
int SonicOSGeneral::processDefaults(Device *device)
{
...
int SonicOSGeneral::generateConfigSpecificReport(Device *device)
{
	// Variables...
	...
	// Serial Number...
	device->addTableData(paragraphPointer->table, i18n("Serial Number"));
	device->addTableData(paragraphPointer->table, serialNumber.c_str());

	return errorCode;
}


int SonicOSGeneral::generateSecuritySpecificReport(Device *device)
{
	// Variables...
	int errorCode = 0;
/*
