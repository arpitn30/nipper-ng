...
CiscoPIXDevice::CiscoPIXDevice()
{
	deviceMake = "Cisco";
	deviceType = "PIX Security Appliance";
	deviceOS = "PIX";

	administration = new (CiscoPIXAdministration);
}
...
int CiscoPIXDevice::processDeviceSpecific(ConfigLine *command, char *line, int lineSize)
{

	// Version (in general)...
	if ((strcmp(command->part(0), "PIX") == 0) && (strcmp(command->part(1), "Version") == 0))
	{
...
bool CiscoPIXDevice::isDeviceType()
{
	// Variables...
	ConfigLine command;
	char line[1024];
