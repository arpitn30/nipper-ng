...
// Initialisation...
ScreenOSGeneral::ScreenOSGeneral()
{
}


int ScreenOSGeneral::processDeviceConfig(Device *device, ConfigLine *command, char *line, int lineSize)
{
	// Variables...
	int tempInt = 0;
	bool setting = false;
	...
	{
		if (device->config->reportFormat == Config::Debug)
			printf("%sHostname Prompt Line:%s %s\n", device->config->COL_GREEN, device->config->COL_RESET, line);
		tempHostname.assign(command->part(0));
		tempHostname.resize(tempHostname.length() - 2);
	}
...
int ScreenOSGeneral::processDefaults(Device *device)
{
	if (hostname.empty())
...
int ScreenOSGeneral::generateConfigSpecificReport(Device *device)
{
	// Variables...
	int errorCode = 0;
/*
...
int ScreenOSGeneral::generateSecuritySpecificReport(Device *device)
{
	// Variables...
	int errorCode = 0;
/*
